    ...
    for r in results:
        ...
        if len(boxes) > 0:
            ...
            for idx, box in enumerate(boxes):
                ...
                # Draw bounding box + label (always visible)
                ...
                if conf < CONFIDENCE_THRESHOLD:
                    print(f"Ignored low-confidence detection ({conf:.2f})")
                    continue

                # Proceed only if detection is reliable
                print(f"Detected book {idx+1} at ({x1},{y1},{x2},{y2}) with confidence {conf:.2f}")
